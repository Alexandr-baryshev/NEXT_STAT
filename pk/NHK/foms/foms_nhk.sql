            SELECT
              CE.REG_NO,
              (CASE
                WHEN ce.call_numb IS NULL OR ce.call_numb = 0
                THEN substring(CE.ID::varchar, 1, 8)
                ELSE ce.call_numb::varchar END
                ) NUSL,
              ...
              (
                CASE WHEN MIN(CT.RECEIVE_TIME) IS NULL THEN
                  MIN(CE.TIME_REG)
                ELSE
                  MIN(CT.RECEIVE_TIME)
                END
              ...
              (
                CASE WHEN MIN(CT.AT_PLACE_TIME) IS NULL THEN
                  MIN(CE.TIME_REG)
                ELSE
                  MIN(CT.AT_PLACE_TIME)
                END
              )                                           arr_time,
              (
                CASE WHEN MIN(CT.RETURN_TIME) IS NULL THEN
                  MIN(CE.TIME_REG)
                ELSE
                  MIN(CT.RETURN_TIME)
                END
	    ...
	    LEFT JOIN TECHNIKA T ON CT.ID_TECH = T.ID
            WHERE
...
/*            #if ($params.get("Выгружено для ФОМС").equals("1"))
              ...
              -- ce.TIME_REG BETWEEN TIMESTAMP '2022-10-03 00:00:00' AND TIMESTAMP '2022-10-04 00:00:00' AND

              --  AND  AC.IS_EXPORTED_TO_FOMS <> 1



                AND ce.REG_NO = 517274

                AND vw_sc_repair_id_pult.id_pult IN (577)
...
/*            #if ($params.get("Только по причине завершения") && !$params.get("Только по причине завершения").isEmpty())
	          ...
	          #if ($params.get("Район выезда") && !$params.get("Район выезда").isEmpty())
              and T.ID_PULT  in ( $params.get("Район выезда"))
	          #end*/
            ...
            GROUP BY
              ...
              ACPF.CODE, AC.ACCOMPANYING_SHEET, ST.CODE, ASTFOMS.CODE, CT.RETREAT_TIME, PARENT_AP.LAST_NAME, PARENT_AP.FIRST_NAME,
              ...
              AIP1.CODE, AIP2.CODE, AIP3.CODE, APD.CAPTION, APD.IS_LPU, CE.REG_NO, AP.ID, ST.PENSION_NUMBER, ASTFOMS.IS_CHILD_PROFIL,
              AP.DOC_DATE_ISSUE, AP.DOC_ORGANIZATION_ISSUE, pult_take_call.caption
